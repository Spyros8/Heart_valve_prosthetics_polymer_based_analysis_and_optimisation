...
from math import *
#FILE 442 DOCS
file442_1storder= 'C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/SAMPLE 11/file784cooling.xlsx'
file='C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/SAMPLE 11/file784ratioscooling.xlsx'
...
plt.grid()
ax1.set_title('FILE 784 sample 11 thickness 1.46-1.51 mm')
plt.show()
...
plt.title('FILE 784 sample 11 thickness 1.46-1.51 mm')
plt.xlabel('Frame')
...
plt.title('FILE 784 sample 11 thcikness 1.46-1.51 mm')
plt.xlabel('Frame')
...
(ax1, ax2) = axs
fig10.suptitle('FILE 784 sample 11 thickness 1.46-1.51 mm')
...
dfsr3= (df2skinroot3.iloc[670:735,:] - 110)
dfsr3[dfsr3 < 0] = 0
(df2skinroot3.iloc[670:735,:] - 110).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for skin at 180 degrees ' )
plt.show()
...
for i in range(1, d+1):
    ...
    x_data, u_data[i] = (dfskinroot3['DEG'].loc[670:734], dfsr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfs1[dfs1 < 0]=0
dfs1.iloc[630:890,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for skin ' )
plt.show()
...
for i in range(1, e+1):
    ...
    x_data, n_data[i] = (dfskin1['DEG'].loc[630:889], dfsr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfcr3= (df2coreroot3.iloc[350:480,:] - 110)
dfcr3[dfcr3 < 0] = 0
(df2coreroot3.iloc[350:480,:] - 110).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core ' )
plt.show()



#all core root 3
dfcr3clean = (df2coreroot3 - 110)
dfcr3clean[dfcr3clean < 0]=0
dfcr3clean.plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core overall ' )
plt.show()

dfcr32 = dfcr3clean.iloc[1050:1200, :]
...
for i in range(1, g+1):
    ...
    x_data, o_data[i] = (dfcoreroot3['DEG'].loc[350:479], dfcr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfc1 = (df2core1-280)
dfc1[dfc1 < 0]=0
dfcr1 = dfc1.iloc[180:590, :]

#core1st order 270 deg
dfcr12 = dfc1.iloc[900:1300, :]
...
dfc1[dfc1 < 0]=0
dfc1.iloc[180:590,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for core ' )
plt.show()
...
for i in range(1, h+1):
    ...
    x_data, v_data[i] = (dfcore1['DEG'].loc[180:589], dfcr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
ax1.set_xlabel('Frame')
plt.grid()
ax1.set_title('FILE 784 sample 10 thickness 1.46-1.51 mm')
plt.show()
...
for i in range(1, (x+1)):
    #print(i)
    f = i-1
    df_listcore1[i-1] = dfcr1.index[(dfcr1.iloc[:, i-1] <= 1.05*dfhalfmaxcore1[i]) & (0.95*dfhalfmaxcore1[i] <= dfcr1.iloc[:, i-1])]
...
core1maxanglesss[core1maxanglesss == core1maxanglesss[11]]=0
core1minanglesss = np.array(core1minangless)
core1minanglesss[core1minanglesss == core1minanglesss[11]]=0
print(core1minanglesss[0])
FWHMcore1 = np.subtract(core1maxanglesss, core1minanglesss)
FWHMcore1 = np.where((FWHMcore1 > 50), nan, FWHMcore1) 
FWHMcore1 = np.where((FWHMcore1 < -50), nan, FWHMcore1)       
...
plt.figure(figsize = (12, 8))
plt.title('1st order core FWHM sample 11 cooling')
...
for i in range(1, (l+1)):
    #print(i)
    f = i-1
    df_listcore3[i-1] = dfcr3.index[(dfcr3.iloc[:, i-1] <= 1.08*dfhalfmaxcore3[i]) & (0.92*dfhalfmaxcore3[i] <= dfcr3.iloc[:, i-1])]
...
core3maxanglesss[core3maxanglesss == core3maxanglesss[161]] = 0
core3minanglesss = np.array(core3minangless)
core3minanglesss[core3minanglesss == core3minanglesss[161]] = 0
print(core3minanglesss[0])   
print(core3maxanglesss[0])
FWHMcore3 = np.subtract(core3maxanglesss, core3minanglesss)
FWHMcore3 = np.where((FWHMcore3 > 50), nan, FWHMcore3) 
FWHMcore3 = np.where((FWHMcore3 < -50), nan, FWHMcore3)       
plt.figure(figsize = (12, 8))
...
plt.figure(figsize = (12, 8))
plt.title('root 3 core FWHM sample 11 cooling')
...
for i in range(1, (r+1)):
    #print(i)
    f = i-1
    df_listskin1[i-1] = dfsr1.index[(dfsr1.iloc[:, i-1] <= 1.05*dfhalfmaxskin1[i]) & (0.95*dfhalfmaxskin1[i] <= dfsr1.iloc[:, i-1])]
...
#skin1maxanglesss[skin1maxanglesss == skin1maxanglesss[196]]=0
skin1minanglesss = np.array(skin1minangless)    
#skin1minanglesss[skin1minanglesss == skin1minanglesss[196]]=0
...
plt.figure(figsize = (12, 8))
plt.title('1st order skin FWHM sample 11 cooling')
...
for i in range(1, (s+1)):
    #print(i)
    f = i-1
    df_listskin3[i-1] = dfsr3.index[(dfsr3.iloc[:, i-1] <= 1.18*dfhalfmaxskin3[i]) & (0.82*dfhalfmaxskin3[i] <= dfsr3.iloc[:, i-1])]
...
#skin3maxanglesss[skin3maxanglesss == skin3maxanglesss[107]] = 0
skin3minanglesss = np.array(skin3minangless)
#skin3minanglesss[skin3minanglesss == skin3minanglesss[107]] = 0
...
plt.figure(figsize = (12, 8))
plt.title('root 3 skin FWHM sample 11 cooling')
