...
#FILE 442 DOCS
file442_1storder= 'C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 10/FILE780.xlsx'
file='C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 10/file780rastios.xlsx'
...
plt.grid()
ax1.set_title('FILE 780 sample 10 thickness 0.55-0.68 mm')
plt.show()
...
plt.title('FILE 780 sample 10 thickness 0.55-0.68 mm')
plt.xlabel('Frame')
...
plt.plot(frame_array442, file442ratio1, 'g.',  color='g', marker='.', label='FILE 779 sample 10 thickness 0.55-0.68 mm')
plt.title('FILE 780 sample 10 thickness 0.55-0.68 mm')
plt.xlabel('Frame')
...
(ax1, ax2) = axs
fig10.suptitle('FILE 780 sample 10 thickness 0.55-0.68 mm')
...
dfsr3= (df2skinroot3.iloc[680:750,:] - 230)
dfsr3[dfsr3 < 0] = 0
(df2skinroot3.iloc[680:750,:] - 230).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for skin at 180 degrees ' )
plt.show()
...
for i in range(1, d+1):
    ...
    x_data, u_data[i] = (dfskinroot3['DEG'].loc[680:749], dfsr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfs1[dfs1 < 0]=0
dfs1.iloc[645:755,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for skin ' )
plt.show()
...
for i in range(1, e+1):
    ...
    x_data, n_data[i] = (dfskin1['DEG'].loc[645:754], dfsr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfcr3= (df2coreroot3.iloc[260:309,:] - 205)
dfcr3[dfcr3 < 0] = 0
(df2coreroot3.iloc[260:309,:] - 210).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core ' )
plt.show()



#all core root 3
dfcr3clean = (df2coreroot3 - 205)
dfcr3clean[dfcr3clean < 0]=0
dfcr3clean.plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core overall ' )
plt.show()

dfcr32 = dfcr3clean.iloc[960:1010, :]
...
for i in range(1, g+1):
    ...
    x_data, o_data[i] = (dfcoreroot3['DEG'].loc[260:308], dfcr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfc1 = (df2core1-220)
dfc1[dfc1 < 0]=0
dfcr1 = dfc1.iloc[210:460, :]

#core1st order 270 deg
dfcr12 = dfc1.iloc[900:1150, :]
...
dfc1[dfc1 < 0]=0
dfc1.iloc[210:460,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for core ' )
plt.show()
...
for i in range(1, h+1):
    ...
    x_data, v_data[i] = (dfcore1['DEG'].loc[210:459], dfcr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
for i in range(1, (x+1)):
    #print(i)
    f = i-1
    df_listcore1[i-1] = dfcr1.index[(dfcr1.iloc[:, i-1] <= 1.1*dfhalfmaxcore1[i]) & (0.9*dfhalfmaxcore1[i] <= dfcr1.iloc[:, i-1])]
...
core1maxanglesss[core1maxanglesss == core1maxanglesss[197]]=0
core1minanglesss = np.array(core1minangless)
core1minanglesss[core1minanglesss == core1minanglesss[197]]=0
...
plt.figure(figsize = (12, 8))
plt.title('1st order core FWHM sample 10 cooling')
...
core3maxanglesss[core3maxanglesss == core3maxanglesss[216]] = 0
core3minanglesss = np.array(core3minangless)
core3minanglesss[core3minanglesss == core3minanglesss[216]] = 0
...
plt.figure(figsize = (12, 8))
plt.title('root 3 core FWHM sample 10 cooling')
...
plt.figure(figsize = (12, 8))
plt.title('1st order skin FWHM sample 10 cooling')
...
skin3maxanglesss[skin3maxanglesss == skin3maxanglesss[175]] = 0
skin3minanglesss = np.array(skin3minangless)
skin3minanglesss[skin3minanglesss == skin3minanglesss[175]] = 0
print(skin3minanglesss[4])
FWHMskin3 = np.subtract(skin3maxanglesss, skin3minanglesss)  
FWHMskin3 = np.where((FWHMskin3 > 15), nan, FWHMskin3) 
FWHMskin3 = np.where((FWHMskin3 < -50), nan, FWHMskin3)     
plt.figure(figsize = (12, 8))
...
plt.figure(figsize = (12, 8))
plt.title('root 3 skin FWHM sample 10 cooling')
