...
#FILE 442 DOCS
file442_1storder= 'C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 8/SAMPLE8COOLINGWITHNOISE.xlsx'
file='C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 8/SAMPLE8COOLINGINDIVIDUAL.xlsx'
...
dfsr3= (df2skinroot3.iloc[635:745,:] - 200)
dfsr3[dfsr3 < 0] = 0
(df2skinroot3.iloc[635:745,:] - 200).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for skin at 180 degrees ' )
plt.show()
...
for i in range(1, d+1):
    ...
    x_data, u_data[i] = (dfskinroot3['DEG'].loc[635:744], dfsr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfs1[dfs1 < 0]=0
dfs1.iloc[565:835,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for skin ' )
plt.show()
...
for i in range(1, e+1):
    ...
    x_data, n_data[i] = (dfskin1['DEG'].loc[565:834], dfsr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfcr3= (df2coreroot3.iloc[250:410,:] - 150)
dfcr3[dfcr3 < 0] = 0
(df2coreroot3.iloc[250:410,:] - 150).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core ' )
plt.show()



#all core root 3
dfcr3clean = (df2coreroot3 - 150)
...
for i in range(1, g+1):
    ...
    x_data, o_data[i] = (dfcoreroot3['DEG'].loc[250:409], dfcr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfc1[dfc1 < 0]=0
dfc1.iloc[160:520,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for core ' )
plt.show()
...
for i in range(1, h+1):
    ...
    x_data, v_data[i] = (dfcore1['DEG'].loc[160:519], dfcr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
for i in range(1, (x+1)):
    #print(i)
    f = i-1
    df_listcore1[i-1] = dfcr1.index[(dfcr1.iloc[:, i-1] <= 1.25*dfhalfmaxcore1[i]) & (0.75*dfhalfmaxcore1[i] <= dfcr1.iloc[:, i-1])]
