...
file='C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 8/SAMPLE8EXTRUSIONINDIVIDUAL.xlsx'
...
dfsr3= (df2skinroot3.iloc[670:735,:] - 150)
dfsr3[dfsr3 < 0] = 0
(df2skinroot3.iloc[670:735,:] - 150).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for skin at 180 degrees ' )
plt.show()
...
dfs1[dfs1 < 0]=0
dfs1.iloc[520:880,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for skin ' )
plt.show()
...
for i in range(1, e+1):
    ...
    x_data, n_data[i] = (dfskin1['DEG'].loc[520:879], dfsr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfcr3= (df2coreroot3.iloc[393:410,:] - 170)
dfcr3[dfcr3 < 0] = 0
(df2coreroot3.iloc[393:410,:] - 170).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core ' )
plt.show()



#all core root 3
dfcr3clean = (df2coreroot3 - 170)
dfcr3clean[dfcr3clean < 0]=0
dfcr3clean.plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core overall ' )
plt.show()

dfcr32 = dfcr3clean.iloc[980:1100, :]
...
for i in range(1, g+1):
    ...
    x_data, o_data[i] = (dfcoreroot3['DEG'].loc[393:409], dfcr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfc1 = (df2core1-210)
dfc1[dfc1 < 0]=0
dfcr1 = dfc1.iloc[165:520, :]

#core1st order 270 deg
dfcr12 = dfc1.iloc[860:1220, :]
...
dfc1[dfc1 < 0]=0
dfc1.iloc[165:520,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for core ' )
plt.show()
...
for i in range(1, h+1):
    ...
    x_data, v_data[i] = (dfcore1['DEG'].loc[165:519], dfcr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
for i in range(1, (l+1)):
    #print(i)
    f = i-1
    df_listcore3[i-1] = dfcr3.index[(dfcr3.iloc[:, i-1] <= 1.05*dfhalfmaxcore3[i]) & (0.95*dfhalfmaxcore3[i] <= dfcr3.iloc[:, i-1])]
