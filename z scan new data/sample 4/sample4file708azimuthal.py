...
#FILE 442 DOCS
file442_1storder= 'C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 4/FILE708.xlsx'
file='C:/Users/Spyros/.spyder-py3/RESEARCH PROJECT RELEVANT IIB/z scan new data/sample 4/FILEFORRATIOSCOOLING.xlsx'
...
plt.title('FILE 708-sample 4 thickness 0.8-0.83 mm')
plt.xlabel('Frame')
...
dfsr3= (df2skinroot3.iloc[660:730,:] - 210)
dfsr3[dfsr3 < 0] = 0
(df2skinroot3.iloc[660:730,:] - 210).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for skin at 180 degrees ' )
plt.show()
...
for i in range(1, d+1):
    ...
    x_data, u_data[i] = (dfskinroot3['DEG'].loc[660:729], dfsr3[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfs1[dfs1 < 0]=0
dfs1.iloc[570:835,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for skin ' )
plt.show()
...
for i in range(1, e+1):
    ...
    x_data, n_data[i] = (dfskin1['DEG'].loc[570:834], dfsr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
dfcr3= (df2coreroot3.iloc[320:410,:] - 210)
dfcr3[dfcr3 < 0] = 0
(df2coreroot3.iloc[320:410,:] - 210).plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core ' )
plt.show()



#all core root 3
dfcr3clean = (df2coreroot3 - 210)
dfcr3clean[dfcr3clean < 0]=0
dfcr3clean.plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = 'Root 3 peaks for core overall ' )
plt.show()

dfcr32 = dfcr3clean.iloc[1010:1100, :]
...
dfc1 = (df2core1-210)
dfc1[dfc1 < 0]=0
dfcr1 = dfc1.iloc[250:450, :]

#core1st order 270 deg
dfcr12 = dfc1.iloc[980:1130, :]
...
dfc1[dfc1 < 0]=0
dfc1.iloc[250:450,:].plot(figsize=[12,12], legend= False, ylim = [0, 1000], title = '1st order peaks for core ' )
plt.show()
...
for i in range(1, h+1):
    ...
    x_data, v_data[i] = (dfcore1['DEG'].loc[250:449], dfcr1[i])
    #xdata =x_data/max(x_data)
    #ydata[i] =y_data[i]/max(y_data[i])
...
for i in range(1, (x+1)):
    #print(i)
    f = i-1
    df_listcore1[i-1] = dfcr1.index[(dfcr1.iloc[:, i-1] <= 1.11*dfhalfmaxcore1[i]) & (0.89*dfhalfmaxcore1[i] <= dfcr1.iloc[:, i-1])]
...
for i in range(1, (l+1)):
    #print(i)
    f = i-1
    df_listcore3[i-1] = dfcr3.index[(dfcr3.iloc[:, i-1] <= 1.14*dfhalfmaxcore3[i]) & (0.86*dfhalfmaxcore3[i] <= dfcr3.iloc[:, i-1])]
...
for i in range(1, (r+1)):
    #print(i)
    f = i-1
    df_listskin1[i-1] = dfsr1.index[(dfsr1.iloc[:, i-1] <= 1.1*dfhalfmaxskin1[i]) & (0.9*dfhalfmaxskin1[i] <= dfsr1.iloc[:, i-1])]
