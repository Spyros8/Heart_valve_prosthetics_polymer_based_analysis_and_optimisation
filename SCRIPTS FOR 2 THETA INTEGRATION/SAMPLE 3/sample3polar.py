...
#REMEMBER SHEET 5 IS THE FRAME CUT ONE
...
fig12.suptitle('SAMPLE 3 1st order peaks')
...
ax4.set_xlabel('2 theta')
ax4.set_ylabel('intensity')
fig13.suptitle('SAMPLE 3 root 3 and 2nd order peaks')
df_list = [df24421storder, df25051storder, df24991storder, df24961storder]
# plot counter
n = 0
for r in range(2):
...
lines15 = df442root3max.plot.line(legend=True, grid = True, label='FILE 442')

df505root3max = df25051storder.iloc[24:33,:].max()

lines16 = df505root3max.plot.line(legend=True, grid = True, label = 'FILE505')

df499root3max = df24991storder.iloc[24:33,:].max()

lines17 = df499root3max.plot.line(legend=True, grid = True, label = 'FILE 499')

df496root3max = df24961storder.iloc[24:33,:].max()

lines18 = df496root3max.plot.line(legend=True, grid = True, label = 'FILE 472')
...
fig15.suptitle('SAMPLE 3 1st order peak max')
ax1.set_title('12 mm from injection pt')
ax1.set_xlabel('frame')
